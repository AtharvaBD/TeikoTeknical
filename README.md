...
├── requirements.txt
├── Makefile
├── avgb_cell.py                # standalone script for the free response question
└── pipeline/
...
Installs everything listed in `requirements.txt` (pandas, scipy, statsmodels,
...
- **Significance testing** uses **Welch's t-test** (does not assume equal variance between
  groups) per population, with **FDR correction** across the 5
  ...
  this correction. Group sizes here are large, so the t-test's
